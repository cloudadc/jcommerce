...
public abstract class GoodsListBase extends ContentWidget {
	...
	public GoodsListBase() {
		add(contentPanel);
		initJS(this);
	}

    public String getButtonText() {
        return virtualCard ? "添加新虚拟卡" : Resources.constants.GoodsList_add_new();
    }
    
    protected void buttonClicked() {
        if (virtualCard) {
            NewVirtualCard.State state = new NewVirtualCard.State();
            state.execute();
        } else {
...
//    	wantedFields.add(IGoods.NUMBER);
        ...
        criteria.addCondition(new Condition(IGoods.REALGOODS, Condition.EQUALS, ""+!virtualCard));
...
//		panel.addButton(new Button(virtualCard ? "添加新虚拟卡" : Resources.constants.GoodsList_add_new(),
//				new SelectionListener<ButtonEvent>() {
//					public void componentSelected(ButtonEvent ce) {
//					    if (virtualCard) {
//                            NewVirtualCard.State state = new NewVirtualCard.State();
...
//				}));
	...
	private native void initJS(GoodsListBase me) /*-{
	   $wnd.deleteGoods = function (id) {
	       me.@com.jcommerce.gwt.client.panels.goods.GoodsListBase::deleteGoodsAndRefrsh(Ljava/lang/String;)(id);
	   };
	   $wnd.editGoods = function (id) {
	       me.@com.jcommerce.gwt.client.panels.goods.GoodsListBase::editGoods(Ljava/lang/String;)(id);
	   };
	   }-*/;
